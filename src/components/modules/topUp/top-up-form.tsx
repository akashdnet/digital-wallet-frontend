...
import { useSendMoneyMutation, useTopUpMutation } from "@/redux/features/wallet/wallet.api";
...
export function TopUpForm({
  ...
  const [sendTopUpError, setSendTopUpError] = useState<string>("");
  const [sendTopUpPost, { isLoading , error }] = useTopUpMutation()
  ...
  const onSubmit: SubmitHandler<FieldValues> = async (data) => {
    ...
    try {
      const res = await sendTopUpPost(data).unwrap();
      toast.success("TopUp sent successfully.", { id: toastId } );
      navigate("/transaction");
    } catch (error:any) {
      toast.error("something went wrong.", { id: toastId });
      setSendTopUpError(error.data.message);
      // console.log(error)
    }
  };
    ...
    >
      <Card className="max-w-md w-full mx-auto ">
        <CardHeader className="text-center space-y-4">
          <CardTitle className="text-2xl font-bold">TopUp Securely</CardTitle>
        </CardHeader>
          ...
          <Form {...form}>
            ...
            >
                  ...
                  <FormItem>
                    ...
                    <FormControl>
                      <Input
                        ...
                        onChange={(e) => {
                        field.onChange(e);
                        if (sendTopUpError) setSendTopUpError("");
                      }}
                        required
                        minLength={8}
                      />
              ...
              <Button type="submit" disabled={isLoading} className="w-full cursor-pointer">
                {isLoading ? "Sending..." : "TopUp Now"}
              </Button>
            </form>
          </Form>
          <h1 className="text-red-500 text-center">{sendTopUpError}</h1>
        </CardContent>
